...
if 'uknl_maybe_inject_ttyusb' not in s:
    ...
    helpers = (
        ...
        'static bool uknl_query(const char *name, const char *req, char *resp, size_t resp_sz)\n'
        '{\n'
        '\tint s = socket(AF_UNIX, SOCK_STREAM, 0);\n'
        '\tif (s < 0)\n\t\treturn false;\n'
        '\tstruct timeval tv = { .tv_sec = 1, .tv_usec = 0 };\n'
        ...
        '\tchar list[256];\n'
        '\tif (uknl_query("io.neoterm.ttyusb", "LIST\\n", list, sizeof list) && list[0] != \'\\0\' && list[0] != \'-\') {\n'
        '\t\tchar *save = NULL;\n'
        ...
        '\t\t\tif (r == 0) break;\n\t\t\toff += r;\n\t\t}\n'
        '\t}\n'
        '\t/* uksd0 only when a mass-storage device is actually attached: io.neoterm.block\n'
        '\t * answers SIZE with OK only while deviceName != null, so it hotplugs like ttyUSB. */\n'
        '\tchar bresp[64];\n'
        '\tif (getenv("UK_BLOCK") && uknl_query("io.neoterm.block", "SIZE\\n", bresp, sizeof bresp)\n'
        '\t    && bresp[0] == \'O\' && bresp[1] == \'K\') {\n'
...
if 'uknl_block_dispatch' not in s:
    ...
    helpers = (
        ...
        'static int uksd_wn(int s, const void *b, size_t n) { size_t o = 0; while (o < n) { ssize_t r = write(s, (const char *)b + o, n - o); if (r <= 0) return -1; o += r; } return 0; }\n'
        'static int uksd_rn(int s, void *b, size_t n) { size_t o = 0; while (o < n) { ssize_t r = read(s, (char *)b + o, n - o); if (r <= 0) return -1; o += r; } return 0; }\n'
