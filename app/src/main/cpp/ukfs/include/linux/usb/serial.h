/* uKernel hamis <linux/usb/serial.h> — a usb-serial mag szerződése.
 * A valódi kernel-header (drivers/usb/serial használja) lecsupaszítva: csak a
 * 4 forrásfájl által ténylegesen elért mezők/prototípusok, a shim stílusában. */
#ifndef _UK_LINUX_USB_SERIAL_H
#define _UK_LINUX_USB_SERIAL_H
...
/* console — kikapcsolva */
static inline void usb_serial_console_init(int minor) { (void)minor; }
static inline void usb_serial_console_exit(void) { }
static inline void usb_serial_console_disconnect(struct usb_serial *serial) { (void)serial; }
...
{
	(void)dev; (void)function; (void)size; (void)data;
}
...
do {									\
	...
	(void)__port;							\
	dev_err(&(usport)->dev, fmt, ##__VA_ARGS__);			\
} while (0)
